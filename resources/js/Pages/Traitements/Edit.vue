<template>
    <AppLayout :title="`Modifier traitement - ${ruche.nom} - ${rucher.nom}`">
        <template #header>
            <div class="space-y-3">
                <!-- Breadcrumbs -->
                <nav class="text-xs text-gray-600 overflow-x-auto">
                    <div class="flex items-center gap-1 whitespace-nowrap">
                        ...
                        <span>›</span>
                        <Link :href="route('ruchers.ruches.traitements.index', [rucher.id, ruche.id])" class="hover:text-gray-900">
                            Traitements
                        </Link>
                        <span>›</span>
                        <Link :href="route('ruchers.ruches.traitements.show', [rucher.id, ruche.id, traitement.id])" class="hover:text-gray-900">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                        <h2 class="text-lg sm:text-xl font-medium text-gray-900">
                            Modifier le traitement - {{ ruche.nom }}
                        </h2>
                        <p class="text-sm text-gray-600">
                            Traitement du {{ formatDate(traitement.date_traitement) }}
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white border border-gray-200 rounded-sm p-6 sm:p-8">
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input id="produit_utilise" v-model="form.produit_utilise" type="text"
                                    class="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-900 focus:border-gray-900"
                                    placeholder="Ex: Apiguard, Apivar, sirop de nourrissement..."
                                    required />
                            ...
                            <div>
                                ...
                                <input id="dosage" v-model="form.dosage" type="text"
                                    class="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-900 focus:border-gray-900"
                                    placeholder="Ex: 1 lanière par 5 cadres, 2L de sirop..."
                                    required />
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label for="temperature_application" class="block text-sm font-medium text-gray-900 mb-2">
                        ...
                        <div class="flex flex-col gap-3 pt-4 sm:flex-row sm:justify-end">
                            <Link :href="route('ruchers.ruches.traitements.show', [rucher.id, ruche.id, traitement.id])"
                            ...
                            <button type="submit" :disabled="form.processing"
                                ...
                                <span v-else>Modifier le traitement</span>
                            </button>
                        </div>
                    </form>
...
const form = useForm({
    date_traitement: props.formData?.date_traitement || props.traitement.date_traitement,
    type_traitement: props.formData?.type_traitement || props.traitement.type_traitement,
    produit_utilise: props.formData?.produit_utilise || props.traitement.produit_utilise,
    dosage: props.formData?.dosage || props.traitement.dosage,
    methode_application: props.formData?.methode_application || props.traitement.methode_application,
    temperature_application: props.formData?.temperature_application || props.traitement.temperature_application,
    efficacite: props.formData?.efficacite || props.traitement.efficacite,
    effets_secondaires: props.formData?.effets_secondaires || props.traitement.effets_secondaires || '',
    notes: props.formData?.notes || props.traitement.notes || '',
});
...
// Soumission du formulaire avec PUT
function submit() {
    form.put(route('ruchers.ruches.traitements.update', [props.rucher.id, props.ruche.id, props.traitement.id]));
}
</script>